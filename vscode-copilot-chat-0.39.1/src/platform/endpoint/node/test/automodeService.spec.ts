...
describe('AutomodeService', () => {
	...
	describe('model selection', () => {
		...
		it('should prefer configured Claude Sonnet 4.6 family when available', async () => {
			const openaiEndpoint = createEndpoint('gpt-4o', 'OpenAI');
			const sonnetEndpoint = createEndpoint('claude-sonnet-4-6-20260215', 'Anthropic');
			mockApiResponse(['gpt-4o', 'claude-sonnet-4-6-20260215']);

			await (configurationService as InMemoryConfigurationService).setConfig(
				ConfigKey.AutoModePreferredModel,
				AutoModeModelPreference.ClaudeSonnet46
			);
			...
			const chatRequest: Partial<ChatRequest> = {
				location: ChatLocation.Panel,
				prompt: 'test',
				sessionId: 'session-prefer-sonnet'
			};

			const result = await automodeService.resolveAutoModeEndpoint(chatRequest as ChatRequest, [openaiEndpoint, sonnetEndpoint]);
			expect(result.model).toBe('claude-sonnet-4-6-20260215');
		});

		it('should prefer configured Claude Opus 4.6 family over router decisions', async () => {
			const openaiEndpoint = createEndpoint('gpt-4o', 'OpenAI');
			const opusEndpoint = createEndpoint('claude-opus-4.6-20260215', 'Anthropic');
			mockApiResponse(['gpt-4o', 'claude-opus-4.6-20260215']);
			...
			await (configurationService as InMemoryConfigurationService).setConfig(
				ConfigKey.AutoModePreferredModel,
				AutoModeModelPreference.ClaudeOpus46
			);
			...
			(mockFetcherService.fetch as any).mockResolvedValue({
				...
				json: vi.fn().mockResolvedValue({
					...
					latency_ms: 10,
					chosen_model: 'gpt-4o',
					candidate_models: ['gpt-4o', 'claude-opus-4.6-20260215'],
					scores: {
						needs_reasoning: 0.1,
						no_reasoning: 0.9
					}
			...
			const chatRequest: Partial<ChatRequest> = {
				location: ChatLocation.Panel,
				prompt: 'test',
				sessionId: 'session-prefer-opus'
			};

			const result = await automodeService.resolveAutoModeEndpoint(chatRequest as ChatRequest, [openaiEndpoint, opusEndpoint]);
			expect(result.model).toBe('claude-opus-4.6-20260215');
			expect(mockFetcherService.fetch).not.toHaveBeenCalled();
		});
	});
});
