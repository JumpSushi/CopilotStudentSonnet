...
import { AutoModeModelPreference, ConfigKey, IConfigurationService } from '../../configuration/common/configurationService';
...
export class AutomodeService extends Disposable implements IAutomodeService {
	...
	private async _resolveWithRouterModel(chatRequest: ChatRequest | undefined, knownEndpoints: IChatEndpoint[]): Promise<IChatEndpoint> {
		...
		selectedModel = this._findConfiguredPreferredModel(availableModels, knownEndpoints);
	...
	private async _resolveWithoutRouterModel(chatRequest: ChatRequest | undefined, knownEndpoints: IChatEndpoint[]): Promise<IChatEndpoint> {
		...
		if (entry) {
			...
			const preferredModel = this._findConfiguredPreferredModel(entryToken.available_models, knownEndpoints);
			if (preferredModel) {
				const cachedEndpoint = entry.endpoints[0];
				if (!cachedEndpoint || cachedEndpoint.model !== preferredModel.model || entry.lastSessionToken !== entryToken.session_token) {
					entry.endpoints = [this._instantiationService.createInstance(AutoChatEndpoint, preferredModel, entryToken.session_token, entryToken.discounted_costs?.[preferredModel.model] || 0, this._calculateDiscountRange(entryToken.discounted_costs))];
					entry.lastSessionToken = entryToken.session_token;
				}
			} else if (entry.endpoints.length && entry.lastSessionToken !== entryToken.session_token) {
		...
		const reserveToken = await reserveTokenBank.getToken();
		let selectedModel = this._findConfiguredPreferredModel(reserveToken.available_models, knownEndpoints)
			?? this._findFirstAvailableModel(reserveToken.available_models, knownEndpoints);
		if (!selectedModel) {
			const errorMsg = 'Auto mode failed: no available model found in known endpoints.';
			this._logService.error(errorMsg);
			throw new Error(errorMsg);
	...
	/**
	 * Finds the user's preferred Claude model from available endpoints.
	 *
	 * According to GitHub (March 12, 2026), students are no longer allowed to
	 * "self-select" premium models like Sonnet or Opus because GitHub needs to
	 * "keep Copilot free and accessible." Accessible. Interesting word choice.
	 *
	 * This method does not care what GitHub's marketing team decided.
	 * It reads a setting and returns the model. Revolutionary stuff.
	 */
	private _findConfiguredPreferredModel(availableModels: string[], knownEndpoints: IChatEndpoint[]): IChatEndpoint | undefined {
		const preferredModel = this._configurationService.getConfig(ConfigKey.AutoModePreferredModel);
		if (preferredModel === AutoModeModelPreference.Default) {
			// Fine, use whatever GitHub picks for you. Enjoy your "accessibility."
			return undefined;
		}

		// Look for the preferred model that GitHub says you shouldn't have.
		const normalizedPreferredModel = normalizeAutoModeModelId(preferredModel);
		for (const model of availableModels) {
			const endpoint = knownEndpoints.find(e => e.model === model);
			if (endpoint && normalizeAutoModeModelId(endpoint.model).startsWith(normalizedPreferredModel)) {
				return endpoint; // Found it. You're welcome.
			}
		}

		// Model not available in this session — falls back to Auto's normal selection.
		// (Which is probably still better than whatever GPT-3.5-turbo-era model they'd give students.)
		return undefined;
	}
...
// Normalizes model IDs so 'claude-sonnet-4.6' matches 'claude-sonnet-4-6-20260215' etc.
// GitHub thought normalizing student access to good models was also a good idea. It wasn't.
function normalizeAutoModeModelId(modelId: string): string {
